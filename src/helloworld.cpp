...
vec get_fit(mat generation) {
    ...
    for (int i = 0; i < generation.n_rows; i++) {
        ...
        fit(i) = (double) simulator.walking_challenge(creature, 0);
    }
    return fit;
}
...
//        Create children from the parents
        ...
        for (int j = 0; j < generation.n_rows; j++) {
            ...
            generation.row(j).tail(cross_gene) = generation.row(I_parents_2(j)).tail(cross_gene);
        }
...
//    Look at the best one
    ...
    float best_fit = simulator.walking_challenge(best_creature, &renderer);
